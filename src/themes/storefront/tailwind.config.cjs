/**
 * Storefront theme — Tailwind config.
 *
 ...
 * Flexweg as `/theme-assets/storefront.css`.
 *
 * Content scan is intentionally LOCAL to this theme. The `.js` glob
 * covers the runtime loaders (menu-loader.js, posts-loader.js,
 * catalog-loader.js) that inject DOM whose class names are only
 * referenced from those files — required so Tailwind's purge step
 * doesn't strip the matching @layer components rules.
 *
 ...
 *
 * Palette is anchored on a sage-green primary + terracotta secondary,
 * the organic-minimalism aesthetic from the catalog mockups' DESIGN.md.
 *
 * @type {import('tailwindcss').Config}
 */
module.exports = {
  content: ["./src/themes/storefront/**/*.{ts,tsx,html,js}"],
  safelist: [
    "material-symbols-outlined",
    { pattern: /^archives(__[a-z-]+|-link)?$/ },
    { pattern: /^storefront-(stock|catalog|filter|product|related|bio)-/ },
  ],
  theme: {
    extend: {
      colors: {
      ...
      fontFamily: {
        // Resolved via CSS variables so compileCss() can swap fonts
        // without rebuilding. Mockup uses Playfair Display + Inter.
        serif: ["var(--font-serif)", "Georgia", "serif"],
        sans: ["var(--font-sans)", "Inter", "system-ui", "sans-serif"],
      },
      // Spacing tokens lifted from the catalog mockup design system.
      spacing: {
        "stack-sm": "0.5rem",
        "stack-md": "1rem",
        "stack-lg": "2rem",
      ...
      fontSize: {
        ...
        button: ["0.75rem", { lineHeight: "1.25", letterSpacing: "0.1em", fontWeight: "600" }],
        "body-md": ["1rem", { lineHeight: "1.5", letterSpacing: "0", fontWeight: "400" }],
        "body-lg": ["1.125rem", { lineHeight: "1.6", letterSpacing: "0", fontWeight: "400" }],
        "headline-sm": ["1.5rem", { lineHeight: "1.4", letterSpacing: "0", fontWeight: "600" }],
        "headline-md": ["2rem", { lineHeight: "1.3", letterSpacing: "0", fontWeight: "600" }],
