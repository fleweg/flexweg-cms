...
//     invalidates every cached query) — for an admin tool with a
//     handful of active subscriptions, it's fine.
//   - The poller stops automatically when there are no active
//     subscribers, so an idle modal closes its sub and traffic drops
//     to zero.
...
async function poll(): Promise<void> {
  ...
  try {
    ...
    // react (revoked token = banner "session expired" or similar).
